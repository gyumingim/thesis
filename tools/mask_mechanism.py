...
def main():
    ...
    print("  이탈 → 충돌로만 바뀐 것 %d건 (셋째 차를 «없는 셈» 친 결과라면 여기에 나타난다)"
          % conv)
    print("\n판정: (i) 폭주 제거는 이탈 감소로 %s."
          % ("나타난다" if dO < -5 else "나타나지 않는다"))
    print("(ii) 미학습 잔존은 여기서 **판정하지 않는다** — 충돌이 남는다는 것은 (ii)와")
    print("양립할 뿐 시험이 아니다. 남은 충돌을 «없는 셈 친 셋째 차» 에 귀속시키려면")
    print("노출 층화가 필요한데, bench/mask_threat_probe.py 가 세 정의 모두에서 그것이")
    print("순환·길이 교란·빈 층으로 무너짐을 보였다. PAPER §8 (10) 참조.")
    return 0
