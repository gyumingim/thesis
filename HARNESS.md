...
- **역할 분담**: **계획·진단·설계·검토·구현(코드·파일 변경) = 메인(Opus)이 직접.** 필요 시 **조사·서치(웹·코드 탐색) = Sonnet 하위 에이전트**(`model: sonnet`), **무거운 적용·병렬 구현 = Opus 하위 에이전트**(`model: opus`)에 위임.
- 멀티 에이전트(병렬 다중 에이전트)는 사용자가 허용했을 때 자유롭게 사용 — **현재 허용됨**. GPU 렌더는 한 번에 하나(에이전트는 kit.exe 폴링으로 상호 양보).
