    ...
    while len(labels) < n_veh and tries < 150:   # 새 기각 경로가 시도를 더 쓴다
        ...
        if mode < 0.34 and placed:
            ...
            if mode < 0.88:
                ...
                # ★ 2026-09-02: 이 경로를 «역주행을 만든다» 며 도로축 밖으로 제한했다가
                #   **되돌렸다** — 근거로 삼은 «8.5%» 측정이 내 감사 스크립트의 좌표
                #   오류였기 때문이다(카메라 요각 항 누락, 110 m 에서 최대 11 m).
                # ★ 2026-09-05: 변환을 고친 감사(ue/label_audit.py)로 다시 재니 **0 이 아니라
                #   190 중 5(2.6%)** 다. 현상 자체는 실재하고 크기만 8.5% → 2.6% 로 작았다.
                #   그래서 이번에는 **훨씬 좁게** 막는다: 자유 요각을 통째로 제한하지 않고,
                #   «도로축에 정렬(±20°)» 이면서 «차로 중심 ±0.6 m 안» 인 조합만 버린다.
                #   그 조합만이 «회전 중» 으로 읽히지 않고 역주행으로 읽히기 때문이다 —
                #   비스듬히 선 차(무단 주차)나 차로 사이에 걸친 차는 그대로 둔다.
                _t = math.radians(yaw)
                ...
                _a = abs(((yaw + 180) % 360) - 180)
                if min(_a, abs(_a - 180)) < 20.0:            # 도로축 정렬인가
                    _near = min(LANE_CTRS, key=lambda c: abs(c - y))
                    if abs(y - _near) < 0.6 and (y > 0) == (_a > 90):
                        continue                              # 차로 위 역주행 — 버린다
