    ...
    while len(labels) < n_veh and tries < 150:   # 새 기각 경로가 시도를 더 쓴다
        ...
        if mode < 0.34 and placed:
            ...
            if mode < 0.88:
                ...
                # ★ 2026-09-02: 한때 이 경로를 «역주행 차를 만든다» 며 도로축 ±25° 밖으로
                #   제한했다가 **되돌렸다**. 근거로 삼은 «차로 위 차량의 8.5% 가 역주행»
                #   측정이 틀렸기 때문이다 — 라벨의 상대 y 를 세계 y 로 되돌릴 때 카메라
                #   요각(±6°)을 빼먹었고, 거리 110 m 에서 그 오차가 최대 11 m 라 멀쩡한
                #   차의 차로와 방향을 뒤바꿔 읽었다. 회전행렬을 제대로 적용해 다시 세면
                #   수정 전·후 모두 **역주행 0건**이다. 자유 요각은 방향을 모르는 배치의
                #   정직한 사전분포이므로 U(0,360) 을 유지한다(부록 A.3 의 좌표 함정 7).
                _t = math.radians(yaw)
