> **[2026-08-28 정정 고지]** 본 논문의 전이 실험에 관측 좌표 부호 규약 오류가 있었다. 경량 시뮬의 횡방향(가로) 좌표가 MetaDrive 규약과 거울상이어서, 전이 평가에서만 좌우가 반전된 관측이 정책에 입력되었다(§4.7). 정정 후 재학습·재평가한 결과 전이 성공률은 0% 에서 **48.9%±20.4%**(정숙 조건 136M steps, 3시드)로 회복되었으나 네이티브 68%±9% 에는 여전히 미치지 못한다. 즉 부호 오류는 결론의 **방향**이 아니라 **크기**를 왜곡했다 — 완만한 열세를 완전 붕괴로 4배 이상 과장한 것이다. 그 위에 세워졌던 §6.3(착취 메커니즘), §6.5(처방 3종 판정), DPC 실측은 아티팩트로 판명되었다. 해당 절은 사례 기록으로 보존하되 결론에서 제외한다. 영향 범위 전수 감사: docs/REVISION_AUDIT.md.
...
**핵심 실증 결과**: 계층 정합을 마친 경량 시뮬의 제로샷 전이는 1시간 예산에서
**48.9%±20.4%** 로 네이티브 **68%±9%** 에 미치지 못한다(첫 체크포인트인 5분에서 피크 58.9%,
40분에 최저 40.0%, 이후 부분 반등해 최종 48.9%).
그러나 실패의 **크기**는 처음 관측한 것과 전혀 다르다 — 정합에 남아 있던 좌표 부호 규약
오류가 이 완만한 열세를 **성공률 0%(이탈 95%)의 완전 붕괴로 과장**하고 있었다(§4.7).
...
디퓨전 정제가 실사 차량 검출을 +7.2pp, 실사 25장 파인튜닝이 +47.7pp 개선했고, 학습된 정책
(GPU 경합 조건 20M-step 체크포인트, §6.6)은 제3의 시뮬레이터(CARLA)에서 폐루프 주행에
성공했다.
...
**전반부 답: 이기지 못한다 — 그러나 완패도 아니다.** 정합을 마친 경량 시뮬의 제로샷
전이는 48.9%±20.4% 로 네이티브 68%±9% 에 못 미친다(5분 피크 58.9% → 40분 40.0% → 최종 48.9%).
...
5. **원리의 교차 도메인 검증(§6.4, §6.6)** — 절차 생성 장면의 디퓨전 정제가 실사 차량 검출을
   +7.2pp, 실사 25장 파인튜닝이 +47.7pp 개선했고, 학습된 정책(GPU 경합 조건 20M-step
   체크포인트, §6.6)은 제3 시뮬레이터(CARLA)에서 폐루프 주행에 성공했다(전이 성립 3층위의
   절제 포함).
...
관찰한 Pan et al. 2201.03544)가 이 그림을 보완한다. 본 연구는 이 계보에서 **자기 실증을 잃었다**: 착취로
...
압축해 §4.1 에 함께 싣고(§4.2~§4.6 은 별도 절을 두지 않는다), 일곱 번째 라운드만
핵심 기여이므로 §4.7 로 상술한다.
...
회귀 모드(`--inject-mirror`)를 두었고, 이 경우 감사는 해당 필드를 우(+)로 정확히 되짚으며
종방향 필드는 좌(+)로 남는다. 이 스크립트를 처음부터 돌렸다면 6주가 아니라 1분에
잡혔을 결함이다.
...
구조적 이유이며, 동시에 "in-domain 지표로는 정합 오류를 검출할 수 없다"는 실무 함의다. 원자료: bench_results/fixed/, 브랜치 origin/fix-lateral-sign.
...
48.9% 에 머무르므로(§6.2), **필요조건이되 충분조건이 아니라는 결론 자체는 유지되나
그 잔여 격차의 기제는 미규명**이다.

**검증 (b) — 퇴화 차원 마스킹 (완료)**: 재학습 없이 평가 시 퇴화 차원(학습 σ<10⁻³)만
0으로 마스킹하면 →MetaDrive 전이가 크게 회복된다. **부호 정정판 재측정(2026-08-28, V=2 2시드)**: 전이 10%/0%(평균 5%)가 마스킹 후 33%/77%(**평균 55%**)로 회복되어, V=3 정상 학습의 확정 전이(48.9%±20.4%, §6.2)와
구간이 완전히 겹친다 — 다만 2시드·σ=31%p 로 분산이 커 우열은 논할 수 없고, 결손의 **주된
원인이 지지집합 위반이었다**는 점만 확증한다. (부호 오류판의 기존 수치 5%→13.5% 는 거울상 관측이 구제 효과를 가린 값이었다.) in-domain 은 86%/89% 로 정상이므로, 차이는 전적으로 관측 지지집합 결손에서 온다. 즉 마스킹은
...
ᵃ 40에피소드 세트, ᵇ 20에피소드 세트. 핵심 대조(3→4)는 동일 세트(ᵇ) 내 비교이므로 유효하다.
...
| **학습 포함 (확정 본판 실측, 데스크톱 정숙)** | custom 37.8K(136M steps) vs 네이티브 604(2.17M steps) | **62.6×** |

**처리량 수치의 층위를 구분한다.** 144배는 학습 루프 없는 순수 환경 스텝 비, 53배는
노트북에서 학습을 포함한 값, **62.6배는 확정 본판(데스크톱 정숙 조건)이 실제로 실현한
비율**이다. 층위에 따라 값이 크게 달라지므로 단일 배수를 논문의 대표값으로 쓰지 않으며,
어느 층위에서도 이 우위가 전이 성능으로 온전히 환산되지 않는다는 점이 §6.2 의 결과다.
...
| 경량 V=2 + 퇴화차원 마스킹 (정정판) | **55% ± 31%** (33/77, 2시드) | §5 검증 (b) — 구제 효과. 2시드·σ 31%p 로 아래 확정 행(48.9%)과 구간이 겹치며 순위 해석은 불가 |
| (참고) 같은 조건 부호 오류판 | 13.5% ± 5% | **아티팩트** |
| **경량 V=3 (정정판·정숙 조건, 1h 최종)** | **48.9% ± 20.4%** (27/67/53, 3시드, 136M steps) | 확정치 |
| 경량 V=3 (정정판·GPU 경합 조건) | 67.8% ± 19.0% (83/73/47, 20M steps) | 샘플 1/7 — 하강 구간 미도달 |
| 경량 V=3 (부호 오류판) | IQM 0% [0,18] (5시드) | **아티팩트** — §4.7 사례로만 인용 |
| 경량 V=3 최적 조기정지 (오류판, 5시드) | 30% ± 18% | **아티팩트**(오류판 수치) — 다만 정정판에서도 사후 오라클 조기정지는 62.2%(50/83/53)로 최종 48.9% 를 13.3pp 상회하므로 **조기정지의 유효성 자체는 정정 후에도 살아 있다** |
| (참고) 파일럿 R5 30분 | 30% (단일 시드) | 오류판 시대 파일럿 — 참고용 |
...
즉 피크는 첫 체크포인트이고 이후 40분까지 완만히 하강한 뒤 부분 반등해 최종에 이른다.
...
이탈 8.9% → 30%, 충돌 32% → 26%). 짧은 20M 조건에서는 이 이동이 나타나지 않는다 —
하강 구간에 도달하기 전에 예산이 끝나기 때문이다.
...
12,857s 시점이 됨 → 벽시계 3,600s 시점 상태인 final 을 그대로 사용했다(대체하지 않음; t=2,700s 체크포인트는 77%)
(부록 A; final 사용 시 70%, 3시드 평균 63%±6%로 결론 불변).
² s2는 CUDA 순간 결함으로 11분에 사망(체크포인트 미생성), V=3 본판에서 대체.

**역방향 비대칭 (⚠ **아티팩트 — §4.7 의 역방향 대조 실험과 동일한 조건**): MetaDrive 네이티브 정책 → 경량 환경(V=3)은 2%±3%(0/5/0)로 전멸,
...
이 값은 '전이 가능성의 비대칭'이 아니라, 네이티브 정책이 거울상 관측을 받은 결과다. §4.7 판별 프로토콜 1(네이티브 + 보정 55%→0%)과 같은 현상이며, 규약 불일치의 증거로만 읽어야 한다.
...
애초에 전이 성능의 체크포인트 순위를 예측하지 못한다"는 것뿐이다. 다만 사후 오라클
조기정지가 여전히 13.3pp 이득을 준다는 사실(§6.2)은, 순위 예측이 불가능해도 **분산이
큰 구간을 피하는 선택**에는 값이 있음을 뜻한다. **신규성의 경계(적대
...
표준편차 1/33(COCO 출발은 학습 자체가 복불복). (2) 50장부터 격차가 소멸한다 — 사전학습
...
발판을 제공한다 — 행동 트랙에서 규약·지지집합 정합이 갭을 닫은 것과 같은 층위의 실용적 가치다.
...
- **처방 검증 — (iii) 캐스케이드도 기각 (2026-08-25 실측, 확정)**: 경량 10분
  ...
  60분 66%±10% — 평균 열세에 분산 2배. 시드별로 갈렸다: s1 은 63%(네이티브 66%±10% 와 구간 중첩,
...
- **종합**: 세 처방 중 조기정지만 생존한다. 본 실험 체제(단일 교차로·제로샷/워름스타트)
  ...
  과최적화 붕괴의 행동 지문(§6.3), 그리고 — 판정 자체는 규약 오류 하의 것이라 인용 불가하나 — 그 처방 시험의 설계가 후속 연구가
...
**정책 출처 고지.** 본 절의 모든 CARLA 수치는 **부호 정정판 시드 1 의 20M-step
체크포인트**(GPU 경합 조건, §6.2 둘째 행)로 얻었다. §6.2 의 확정 헤드라인 수치는 정숙
조건 136M-step 이므로 서로 다른 체크포인트이며, 두 체크포인트는 MetaDrive 전이에서
67.8% 대 48.9% 로 갈린다. 따라서 본 절의 절대 성공률을 §6.2 와 직접 비교해서는 안 된다 —
본 절이 분리하는 것은 **어댑터 구성요소의 기여**이며, 그 비교는 동일 체크포인트 안에서만
이루어진다. 두 체크포인트의 CARLA 성능이 실제로 갈리는지는 별도 절제로 측정 중이며
(carla_policy_ab.sh), 본고 제출 시점의 결과는 §8 향후 연구에 기술한다.
...
| 거버너 ON (0.8g) | 마스킹 OFF | 마스킹 ON |
|---|---|---|
| 성공률 | **52%** (21/40) | **95%** (38/40) |
| 실패 내역 | 이탈 13 · 충돌 6(전부 주차 차량) | 이탈 2 |
| 실패 시점 | 19건 중 13건이 교차로 도달 전(진입 속도 미기록), 나머지 6건은 진입 후 23~55 km/h | 진입 54 km/h 고속 구간 |

두 조건은 **오직 6개 내비 곡률 차원**(idx 11,12,13,16,17,18)에서만 다르다. 구성 A 에서는 이
차이가 무의미했으나(94% vs 95%), 관측에 정적 장애물이 들어오자 43pp 로 벌어진다.

![차원별 분포외 진단](figs/fig_ood_dims.png)

**실행 기동별 성능** (거버너 ON + 마스킹, n=180). 조건 라벨은 앵커의 진입/이탈 방위차가
아니라 **실제 실행된 경로의 총 회전각**으로 부여했다 — 경로계획기가 회전 앵커에서도 직진이나
유턴 경로를 내놓는 경우가 있어, 앵커 라벨로 집계하면 통계가 오염된다(부록 A.3-6,7).
...
| 조건 | in-domain | →MetaDrive 전이 |
|---|---|---|
| 기본 학습 (정정판·GPU 경합 20M steps, §6.2 둘째 행) | 81.8% | 67.8% |
| 마찰 한계 학습 | **94%** | **37%** (충돌 37 · 이탈 27) |

두 행 모두 GPU 경합 조건에서 동일 예산으로 얻었으므로 **행 간 비교만 유효**하며,
§6.2 의 확정치(48.9%)와 직접 비교해서는 안 된다.

**결과는 반대였다.** 마찰 한계는 소스 내부 성능을 올렸으나(정책이 더 안전하게 운전한다)
...
비하면 높다 — 두 타깃의 순위가 갈릴 수 있다는 신호다. 거버너 OFF 조건 측정과 시드 정합은
진행 중이며(carla_seed_sweep.sh), 현시점에서는 결론을 내지 않는다.)
...
규약을 맞춘다고 네이티브 수준에 도달하지는 않으며(48.9% vs 68%), 부호 오류는 완만한 열세를
...
- **실무 체크리스트 (본 연구 결과의 함의)**: (0) **규약 검증을 먼저 하라** — 알려진 상대
  ...
  기하 검증이 유일한 탐지 수단이다. (5) **조기정지는 정합을 마친 뒤에도
  유효하다**: 확정 정숙 조건에서 1시간 완주 성능(48.9%)은 사후 오라클 최적 체크포인트
  (62.2%)보다 13.3pp 낮았고, 3시드 중 2시드에서 완주가 중간 체크포인트보다 나빴다(§6.2).
  0% 경착륙은 사라졌으나 '피크 후 완만한 하강'은 남으므로, 전이 목적의 학습에는 경량 시뮬
  완주가 아니라 **주기 체크포인트 + 타깃 소량 평가에 의한 선택**을 권고한다.
...
이기는가 — **이기지 못한다**: 정합을 마친 경량 시뮬의 제로샷 전이는 48.9%±20.4% 로 네이티브
68%±9% 에 못 미치며, 첫 체크포인트(5분, 58.9%)가 피크다. 그러나 이 결론의 **크기는
정합 품질에 극도로 민감하다**: 단일 부호 규약 오류가 같은 조건을 0%(이탈 95%)로 보이게 했고,
...
파인튜닝 +47.7pp) 및 제3 시험장 CARLA 폐루프 주행(20M-step 체크포인트,
docs/CARLA_CLOSED_LOOP.md).
...
판정 자체는 정정 후에도 유지된다. 규약 오류는 완만한 열세를 4배 넘게 과장했을 뿐 부호를
뒤집지는 않았고, 이 구분이 정정 보고의 핵심이다.
...
(4) DPC 의 **최초 실증** — 정정 후 전 구간 τ≈0 이라 본 연구에서는 시간 구조가 관측되지 않았으므로, 다른 시뮬레이터 쌍에서 곡선 형식의 유효성을 처음부터 시험해야 한다. 확정 정숙 조건 136M 체크포인트에 대한 DPC 재산출도 미실시다, (5) bottleneck 등 과제 일반화, (6) **확정 정숙 조건 136M 체크포인트의 CARLA 폐루프 재현** — 현행 CARLA 결과는 전부 20M 경합 조건 체크포인트로 얻었으므로 체크포인트 간 CARLA 성능 차이는 미확정이다(carla_policy_ab.sh), (7) **동일 장비 다시드 네이티브 기준선** — 헤드라인 비교의 장비 교차 문제를 해소한다(bench/run_native_desktop.sh).
