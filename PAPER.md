...
(4) DPC 의 **독립 재현** — 확정 정숙 5시드 재산출에서 신뢰 창 구조는 성립하지 않았고(전반 τ=+0.190 p=0.422, 후반 +0.039 p=0.804, 전 구간 +0.097 p=0.513), 경량 신호의 동적 범위가 평균의 1% 수준이라, DPC 는 실증이 아니라 형식·절차의 제안이다. 다른 시뮬레이터 쌍·다른 과제에서 곡선 형식의 유효성을 처음부터 확인해야 한다. (5) bottleneck 등 과제 일반화, (5-c) **정정판 생성기로 4-arm 재실행** — 합성 학습 장면이 좌측통행이었고(§6.4) 조명 다양성도 이름뿐이었음이(하늘 색이 실제와 반대) 확인됐으므로, 통행 방향·차선 도색·기상 표현을 모두 고친 생성기로 4-arm 을 재실행해야 잔여 갭에서 이 요인들을 분리할 수 있다. 재실행 전 `ue/label_audit.py`(라벨 기하)와 `ue/render_audit.py`(픽셀 외형) 판정을 통과시키는 것을 선행 조건으로 둔다, (5-b) **엔트로피 하한 처방의 시험** — §6.3 의 탐색 붕괴가 소스 수익의 미세 하강 및 전이 성능의 정체(첫 체크포인트 이후 개선 없음)와 관련이 있다면 entropy bonus 스케줄이나 로그표준편차 하한이 직접 처방이 되나 본 연구에서 시험하지 않았다, (6) **두 시험장의 순위 무상관에 대한 기제 규명** — 정책 9종 대조에서 MetaDrive 전이와 CARLA 전이의 상관이 0 과 구별되지 않았다(§6.6; 다만 n=9 의 구간이 [−0.67, +0.66] 로 넓으므로 우선 정책 수를 늘려 추정 자체를 좁혀야 한다). 어느 관측 차원·어느 기동에서 갈리는지는 미규명이다,
