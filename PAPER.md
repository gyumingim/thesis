...
49.3% 에 머무른다(동일 장비 네이티브 62.0%, §6.2). 다만 이 잔여 격차는 유의성이 성립하지 않고 장비 교차
...
| **학습 포함 (데스크톱 정숙, 확정)** | custom 37.8K(136M steps) vs 네이티브 1,096(3.95M steps, 3시드) | **35.9×** |
...
체크포인트를 사후에 고르면 64.7%로 완주 대비 15.4%p 높다.
...
경량 곡선은 평탄하고 네이티브만 올라간다 — 교차는 경량의 하강이 아니라 네이티브의
상승으로 일어난다. 두 밴드가 크게 겹친다는 사실이 1시간 격차가 유의하지 않은 이유다.
...
**확정 정숙 조건(136M, 3시드) 재산출 (2026-08-29, `tools/dpc_recompute.py`)**. 위 두 값은
...
"예측력이 없다"는 결론도 이 과제·이 신호에 한정된다. 다만 여기의 τ≈0 은 GPU 경합 20M 런에서
...
- **실무 체크리스트 (본 연구 결과의 함의)**: (0) **규약 검증을 먼저 하라** — 알려진 상대
  ...
  선택 신호는 타깃에서 와야 한다**: 확정 정숙 조건에서 1시간 완주(49.3%)는 사후 오라클
  최적 체크포인트(64.7%)보다 15.4%p 낮다(§6.2). 다만 64.7% 는 **타깃 라벨을 읽은 상한**이며, 이 상한을
...
- **좌표 규약 오류는 한 번이 아니었다 (2026-08-29 집계)**: 본 연구가 이 결함 유형을 핵심
  ...
  |---|---|---|---|---|
  | 1 | 경량 커널 (§4.7) | 횡방향 기저가 MetaDrive 차량 API 와 거울상 | 전이 0% ↔ 48.9% | **예** (직진 구간) |
