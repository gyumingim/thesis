...
**48.9%±20.4%** 로 네이티브 **68%±9%** 에 미치지 못한다(초반 15분 피크 60% 후 완만한 하강).
...
전이는 48.9%±20.4% 로 네이티브 68%±9% 에 못 미친다(초반 15분 피크 60%, 이후 완만한 하강).
...
보였던 현상이 규약 오류의 산물이었음을 §4.7 이 보인다. 남는 것은 (i) 착취 가설의 검증
절차 — 과제 포화 이전이 아니라 이후에 시작됨을 5분 간격
...
48.9%±20.4%(27/67/53, 정숙 조건 136M steps)로 네이티브 68%±9% 에는 못 미치나, 0% 경착륙은 재현되지 않았다(최종 이탈 0~20%, 전 체크포인트 성공 40~90%).

**통제 확인**: 같은 정책의 **in-domain 성능은 81.8%(83/86/77)로 오류판 82.8%±7.9% 와
...
| 경량 V=2 + 퇴화차원 마스킹 (정정판) | **55% ± 31%** (33/77, 2시드) | §5 검증 (b) — 구제 효과 |
...
| 경량 V=3 최적 조기정지 (오류판, 3시드) | 23% ± 10% (5시드로는 30%±18%) | **아티팩트** — 정정판에는 붕괴가 없어 조기정지 개념이 불필요 |
...
> **[아티팩트 경고]** 본 절의 실측은 부호 오류 하에서 얻어진 것이다. 정정 후 재학습·재평가에서 0% 경착륙과 스폰 직후 조향 커밋 지문은 재현되지 않았다(전 체크포인트 성공률 40~90%, 이탈 0~27%). 다만 s3 은 83%→47% 의 완만한 하강을 보이므로 '하강 자체가 없다'고는 말할 수 없다. 절 전체를 §4.7 의 '위장' 사례로 읽어야 하며, 착취 해석은 철회한다.
...
이루어진다. 두 체크포인트의 CARLA 성능이 실제로 갈리는지는 §6.7 에서 따로 측정한다.
...
깨지는 대가로 CARLA 적합성이 오르는 거래일 가능성은 남아 있으며, 향후 과제로 둔다.)

**함의**: 전이를 성립시키는 조건은 세 층위로 분리된다. (i) **규약 일치**(§4.7) — 없으면 0%,
(ii) **지지집합 점유 패턴 일치**(§5) — 어기면 0%, (iii) **물리 한계의 명시적 표현**(본 절) —
없으면 회전에서 실패. 세 층위의 결손은 성격이 다르다: 규약 불일치와 슬롯 과다 점유는 **즉시 0%** 로 이어지는 반면, 물리 한계 미표현은 **부분 저하**(94%→63%)를 낳는다. 즉 앞의 둘은 필요조건, 셋째는 성능 요인이다.
...
- **실무 체크리스트 (본 연구 결과의 함의)**: (0) **규약 검증을 먼저 하라** — 알려진 상대
  ...
  기하 검증이 유일한 탐지 수단이다. (5) 조기정지는 **일반 처방이 아니다**: 정정판에서는
  1시간 완주 성능이 중간 체크포인트와 같거나 더 좋았다(§6.2). 조기정지는 정합 결함이
  남아 있을 때의 증상 완화책으로만 취급하라.
...
68%±9% 에 못 미치며, 초반 15분 피크(60%) 이후 완만히 하강한다. 그러나 이 결론의 **크기는
