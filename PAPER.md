...
- **시드 축 보강 — 팔당 10시드로 늘렸고, 그 결과 «시드를 늘리는 일» 자체의 함정을 만났다
  ...
  **처리량이 배치 차이를 설명하는가 — 부분만 설명한다 (`tools/steps_vs_batch.py`)**.
  위 해석은 「9월이 더 많이 학습해서 더 잘한다」였다. 검증 가능하다: 체크포인트마다
  `global_step` 과 전이 점수가 저장돼 있으므로 **스텝 수를 맞춰** 두 배치를 맞대면 된다
  (재학습·재평가 없음). 두 배치가 모두 도달한 구간에서 선형보간해 읽으면:

  | 팔 | 스텝 미정렬(각자 끝점) | 스텝 정렬(공통 최대 예산) |
  |---|---|---|
  | 경량 | +12.0%p (p=0.071) | +10.9%p (p=0.216) |
  | 네이티브 | +16.0%p (p=0.051) | +9.6%p (p=0.200) |

  스텝을 맞춰도 차이가 **거의 그대로 남는다** — 특히 경량은 12.0 → 10.9%p 로 9%만 줄어,
  처리량으로 설명되는 몫이 사실상 없다(네이티브는 16.0 → 9.6%p 로 약 40% 준다). 즉
  처음의 「9월이 더 많이 학습해서 더 잘한다」는 해석은 **경량에서 지지되지 않으며**,
  배치 차이의 주된 원인은 3주 사이에 바뀐 **다른 무엇**이고 이 자료로는 그것이 무엇인지
  알 수 없다. 어느 정렬 비교도 유의하지 않으므로(n=5, 단일 블록 30에피소드) **설명되는 몫의
  크기 자체가 불확실하다**. 따라서 실무 지침은 「처리량을 맞추라」가 아니라 **「배치를
  통제하라 — 시드는 이어서 돌고, 나눠 돌았다면 배치를 설계에 넣으라」**가 된다.
  처리량 기록은 **진단 도구**이지 보정 수단이 아니다.

  (이 절의 체크포인트 단위 비교는 b500000 **한 블록** 30에피소드로만 가능하다 — 8블록
  평균은 `final.pt` 에만 있다. 같은 이유로 여기의 수치는 표 6 계열보다 잡음이 크며,
  실제로 b500000 만 보면 배치 간 격차 변화의 **부호가 8블록 평균과 다르게** 나온다.
  이것 자체가 §7 표 6 이 말한 «단일 블록으로 판단하지 말라» 의 한 사례다.)
