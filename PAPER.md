...
남은 두 차원(요각·속도)이 비슷하게 기여한다고 **가정하더라도** 0.45 부근에 그친다 —
이 외삽은 근거가 약하지만, 0.33 에서 1.0 까지의 거리는 차원 두 개로 메워질 폭이 아니다.

**따라서 이 실험은 현재 자료와 정책으로는 해석 가능한 형태로 성립하지 않는다.** 값싼
경로를 모두 재어 닫았다: V 상향은 지지집합 붕괴로, 횡방향 추가는 크기 부족으로, 배율
확대는 질문의 변질로. 남는 것은 **요각 추정기와 속도 필드를 갖춘 CARLA 재수집**인데,
위 외삽이 맞다면 그것으로도 등가에 닿지 못한다. 본 연구가 §8 에 남기는 것은 「해 보라」가
아니라 **「이 과제·이 교통밀도에서는 실제 인지 오차가 정책 행동을 바꾸지 못한다」는
측정 결과와, 그것을 확인한 절차**다. 실험을 의미 있게 하려면 과제 자체가 주변차 상태에
더 민감해야 한다(밀집 합류·교차 협상 등).
