...
### 2.3-신설 예정 절 초안 (2026-08-27 리서치 통합, 사용자 리뷰 대기 — 승인 시 2.4/2.5 로 편입)

**2.4 시뮬레이터 착취와 그 진단.** 최적화가 시뮬레이터의 결함을 파고든다는 관찰은
...
**2.5 소시드 RL 보고 표준.** 본 논문의 평가 보고는 Henderson et al.(1709.06560)이
...
**[브리지 초안 — 사용자 리뷰 대기]** 여섯 라운드가 남긴 것은 "정합 항목의 목록"이
...
**[브리지 초안 — 사용자 리뷰 대기]** 지지집합 원리는 행동 정책에서 도출됐지만 주장
...
**시드 보강 노트 (2026-08-26, 3→5시드, 사용자 리뷰 대기)**: 시드 4·5를 동일 코드·
...
### 6.4 지지집합 원리의 교차 검증: 인지 파이프라인 [제목 변경 — 사용자 리뷰 대기]

**[브리지 초안]** 이 절은 별개 주제가 아니라 §5 원리의 교차 도메인 검증이다: 합성
...
**percept v1 — 사실화 파이프라인의 다운스트림 검증 (2026-08-26 실측, 4-arm)**:
...
- **처방 검증 — (i) 조기정지 정량화 (2026-08-26 소급 분석, 사용자 리뷰 대기)**: 경량
