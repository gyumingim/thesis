...
**정정판에서 다시 세운 메커니즘 — 탐색 붕괴 (2026-08-29, `tools/entropy_collapse.py`)**.
아래 오류판 분석은 "소스 수익은 계속 오르는데 전이만 떨어진다"를 전제로 착취를 주장했다.
확정 정숙 조건의 학습 곡선을 다시 읽으면 **그 전제부터 성립하지 않는다** — 소스 수익도
함께 떨어진다. 초기 램프(0~5분)를 제외한 5~60분 구간에서 완주 4시드를 재면:
