...
- **실무 체크리스트 (본 연구 결과의 함의)**: (0) **규약 검증을 먼저 하라** — 알려진 상대
  ...
  체크포인트가 **같은 30 시나리오**를 본다(아래 «평가 시나리오 축» 항). 시나리오 난이도를
  급내 상관 ρ 로 넣어 다시 돌리면 편향 몫이 ρ=0 에서 97%, ρ=0.3 에서 86%, ρ=0.7 에서
  68% 로 줄지만 관측값은 어느 ρ 에서도 널 구간 안에 남는다 — **판정은 이 가정에
  민감하지 않다**(`tools/oracle_bias.py`). 다만 64.7% 는 **타깃 라벨을 읽은 상한**이며, 이 상한을
...
- **평가 시나리오 축 — 측정했다 (2026-09-02 한계 제기 → 2026-09-11 해소)**: 헤드라인 수치는
  ...
  **블록 간 난이도 차이는 어디서 오는가** (2026-09-16, `tools/scenario_difficulty.py`).
  ...
  독립 가정은 여전히 같은 블록 안
  체크포인트 상관에 관한 것이라 이 측정으로 해소되지 않는다(민감도는 그 항 참조).
