...
착취 메커니즘을 "예측력의 시간 구조"라는 단일 지표로 요약하며, 프록시 과최적화의
Goodhart 구조(Gao et al. 2210.10760)를 시뮬레이터 쌍에서 상관 시계열로 실측한 것이다.
...
정합 실패 지문과 관측 지지집합 원리, (3) 착취의 시간 구조 규명과 그 진단 지표 DPC
(전반 τ=+0.57→후반 τ=-0.03), (4) 처방 3종의 실측 판정(조기정지만 생존), (5) 지지집합
