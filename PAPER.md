...
비해 작아 유의성을 주장할 수 없다**(Welch t=−1.31, df≈7.9, p=0.227). 시드를 3에서 5로
늘려도 p 는 0.385 에서 0.227 로만 이동했다 — 이 격차는 표본을 늘려도 쉽게 유의해지지 않는다. 따라서
...
즉 **n 을 더 늘려도 이 격차는 유의해지지 않을 가능성이 높으며**, 12.7%p 를 "실질적으로 의미
있는 차이"로 볼지는 통계가 아니라 응용이 정할 문제다.
...
**두 시험장의 순위 상관은 0 이다** — Pearson r=−0.008(p=0.98), Spearman ρ=−0.017(p=0.97),
Kendall τ=+0.029(p=0.92). 뒤집힘이 산발적이 아니라 전면적이다: MetaDrive 중위권인 정숙 s3
가 CARLA 에서 유일하게 20/20 을 냈고, MetaDrive 상위권인 20M s2 는 40% 에 그쳤으며,
MetaDrive 최하위권인 마찰 한계 정책이 85% 를 냈다.
...
> 서로 다른 에피소드 집합을 보게 했다. 다만 **모든 정책이 같은 20 에피소드를 본다는
> 대응 설계는 순위 비교에 유리하므로**, 위 상관의 결론은 표본 크기와 별개로 유지된다.
...
| 실행 기동 | 성공률 | n | 경로 최소 반경(중앙) | 진입 속도(중앙) | 필요 횡가속 |
|---|---|---|---|---|---|
| 직진 | 76.4% | 72 | 33.1 m | 46.3 km/h | 0.50 g |
| **우회전** | **54.2%** | 72 | **11.5 m** | 34.0 km/h | **0.79 g** |
| 좌회전 | 77.8% | 27 | 14.9 m | 39.7 km/h | 0.83 g |
| 유턴 | 44.4% | 9 | 9.3 m | 35.1 km/h | 1.04 g |

**실패율이 경로 최소 반경을 따라간다** — 33.1 m→23.6%, 14.9 m→22.2%, 11.5 m→45.8%,
9.3 m→55.6%. 우회전이 좌회전보다 어려운 것은 기하 때문이다: 우측통행에서 우회전은
안쪽 곡선이라 반경이 좌회전의 77% 에 그친다. 필요 횡가속이 0.79 g 로 속도 거버너의
한계(0.8 g)에 딱 붙어 있어, 거버너가 겨우 버티는 구간이 우회전이다.

정책별로 보면 스윕의 성공률 차이가 사실상 전부 이 칸에서 나온다 — 우회전이 8/8 인 정책
(정숙 s3, 20M s1, 20M s3)과 0/8 인 정책(정숙 s5)이 갈리는 동안, **좌회전은 아홉 정책 모두
2~3/3 으로 균일하고 직진도 3~8/8 로 상대적으로 좁다.** 즉 §6.6 이 "물리 한계의 명시적
표현"으로 설명한 층위가 실제로 작동하는 지점은 회전 일반이 아니라 **가장 조인 회전**이며,
정책 간 강건성 차이도 거기서만 드러난다.
...
- **실무 체크리스트 (본 연구 결과의 함의)**: (0) **규약 검증을 먼저 하라** — 알려진 상대
  ...
  선택 편향이다.** 다만 64.7% 는 **타깃 라벨을 읽은 상한**이며, 이 상한을
