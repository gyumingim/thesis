...
(9) **관측 노이즈 주입 실험** — 경량 시뮬의 관측에 인지 파이프라인의 실제 오차를 주입해
학습시키면 전이가 개선되는가. 본 논문은 수행하지 않았으나 **주입할 값은 이제 확정됐다**:
지면평면 접지선 추정의 거리별 잔차가 σ 1.10 / 2.82 / 5.10 m, 평균오차 −0.13 / −0.55 /
−1.72 m 다(§6.4 표). 산포만이 아니라 **편향도 함께** 넣어야 하며, 거리에 따라 커지는
구조를 재현해야 한다. 이 실험은 두 트랙(행동·외형)을 하나의 실험으로 잇는 자리이기도 하다,
