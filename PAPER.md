...
**48.9%±20.4%** 로 네이티브 **68%±9%** 에 미치지 못한다(3시드 평균 곡선 기준 첫 체크포인트인
5분에서 피크 58.9%, 40분에 최저 40.0%, 이후 부분 반등해 최종 48.9%). 다만 n=3·σ=20.4%p 에서
이 격차는 유의하지 않으므로(Welch t=−1.54, df≈2.4, p≈0.24) 점추정 방향의 잠정 판정이다.
...
**전반부 답: 이기지 못한다 — 그러나 완패도 아니고, 확정도 아니다.** 정합을 마친 경량 시뮬의 제로샷
...
|---|---|---|---|
| MetaDrive (네이티브) | 노트북 RTX 4060 · 1h 예산 | **68% ± 9%** (63/57/70/80/70, 5시드; IQM 68% [59,77])¹ | 기준선 |
...
**그림 5.** 확정 정숙 조건(데스크톱, 136M, 3시드)의 벽시계-전이 곡선과 MetaDrive 네이티브
기준선(노트북, 5시드). 음영은 시드 최소~최대. 평균 곡선의 최저점(40분)은 어느 개별 시드의
최저점도 아니지만, 5분 대비 40분의 하락은 세 시드 모두에서 재현된다
(−20.0/−20.0/−16.7%p). **두 곡선은 장비가 다르다**(§7).
...
규약·지지집합 정합으로 0% 붕괴에서 19.1%p 열세까지 좁혔으나 닫지는 못했다
(48.9% vs 68%; §4.7·§5·§6.2)** —
...
**실행 기동별 성능** (거버너 ON + 마스킹; n=180 — 위 구성 A·B 표의 셀과는 **다른 에피소드 집합**이다. 이 풀의 전체 성공은 158/180 = 87.8%, 유턴 제외 140/152 = 92.1%. 원자료는 `/c/carla/` 진화 루프 출력이며 저장소로 이관해야 한다). 조건 라벨은 앵커의 진입/이탈 방위차가
