...
67.8%±19.0%(83/73/47)로 MetaDrive 네이티브 68%±9% 와 통계적 동률이며, 0% 경착륙은 재현되지 않았다(최종 이탈 0~20%, 전 체크포인트 성공 40~90%).
...
| 실패 시점 | 교차로 도달 전(진입 속도 미기록, 중앙값 90스텝) | 진입 54 km/h 고속 구간 |
...
**차원 단위 진단.** 폐루프 중 정책이 실제로 받는 정규화 좌표 z=(obs−μ)/σ 를 기록해
