...
**정정 (2026-08-24 재검증) [사용자 리뷰 대기]**: 위 지면평면 오차는 랜덤 인지
...
- **실무 체크리스트 (본 연구 결과의 함의)**: (1) 교차 시뮬 평가 전 **사전 지지집합
  ...
  실용적 정지 기준이다.
- **경계 착취에 대한 처방 후보 (미실험)**: (i) 조기정지 — 무료지만 상한이 낮다(≈23%);
  (ii) 판정 경계의 보수화(소스의 합법 영역을 타깃보다 엄격하게 수축) — 착취 자체를
  차단, 재학습 1회로 검증 가능; (iii) 경량 사전학습 → 고충실도 미세조정 캐스케이드 —
  §2.2 multi-fidelity 문헌과 접속, 동일 벽시계에서 "네이티브 단독 vs 캐스케이드"가
  차세대 비교 설계.
