...
(3) **1시간 예산의 격차는 유의하지 않다**(p=0.227, 95% CI [−35.0, +9.7]%p). 시드를 3에서
...
- **평가 시나리오 축에 반복이 없다 (2026-09-02 추가)**: 본 논문의 **모든** MetaDrive 전이
  수치는 `--seed 500000 --episodes 30`, 즉 **시나리오 500000~500029 라는 고정된 30장면
  표본 하나**에서 나온다(`bench/run_clean.sh`·`run_native_desktop.sh`·`run_seeds45_desktop.sh`).
  두 팔, 12개 체크포인트, 10개 학습 시드가 전부 같은 30장면을 본다. 따라서 논문의 모든
  오차막대는 **학습 시드 축의 변동**이며, **시나리오 축의 변동은 한 번도 측정되지 않았다**.
  이것이 위 (5) 오라클 판정의 독립 가정이 서는 자리이고(민감도는 그 항에 적었다), 더 넓게는
  49.3% 대 62.0% 라는 두 절대 수치가 다른 30장면 표본에서 얼마나 움직이는지 본 자료로는
  말할 수 없다는 뜻이다. **다만 대응 설계이므로 두 팔의 비교 자체는 시나리오 표본을
  공유해 상쇄된다** — 위협받는 것은 절대 수준의 일반화이지 §6.2 의 대비가 아니다.
  후속 평가가 이 축을 복원할 수 있도록 `bench/evaluate.py` 가 에피소드별 결과와
  시나리오 인덱스를 남기도록 고쳤다(`per_episode` 필드). 기존 JSON 은 집계값만
  담고 있으므로 소급 추정은 불가능하다.
...
(7) **평가 시나리오 축의 복원** — 본 논문의 전이 수치는 전부 시나리오 500000~500029 라는 단일 30장면 표본에 조건부다(§7). 서로 다른 시나리오 블록 여럿에서 같은 프로토콜을 반복해 «시드 간 분산 대 시나리오 간 분산» 을 분해해야 절대 수준을 일반화할 수 있다. 이를 위해 `bench/evaluate.py` 가 에피소드별 결과를 남기도록 이미 고쳤다,
