...
**이동과 의존을 곱해 본다 (2026-09-23, `tools/block_shift.py`)**. 지지집합 논의는 지금까지
«얼마나 어긋났는가» 만 보았다. 그런데 §8 (9) 의 절제 측정이 «정책이 블록마다 얼마나
의존하는가» 를 주었으므로, 둘을 곱해야 **어긋남이 문제가 되는 곳**이 나온다. 동결 통계로
타깃 관측을 표준화한 |z| 와 절제 Δ 를 나란히 두면:

| 블록 | 평균 \|z\| | 의존 Δ(%p) | 이동 × 의존 |
|---|---|---|---|
| ego | 0.66 | 77.7 | **51.2** |
| navi | 0.69 | 58.7 | **40.7** |
| 주변차 위치값(점유 슬롯) | **0.95** | 2.2 | **2.1** |

**가장 크게 어긋난 블록이 가장 덜 중요하다.** 주변차 위치값은 |z|=0.95 로 셋 중 이동이
가장 큰데 의존은 2.2%p 라 곱이 ego 의 1/24 다. 반대로 ego·navi 는 이동이 0.7 안팎으로
작지만 의존이 커서 곱을 지배한다. 별개로 **점유 채널**은 소스 21.2% 대 타깃 14.9%
(**−6.2%p**)로 이동하며, 이 채널의 의존은 34.3%p 다 — 주변차 쪽에서 눈여겨볼 것은
위치의 정확도가 아니라 **몇 개가 보이는가** 다.

이 표는 상관이 아니라 곱이므로 인과를 주장하지 않는다. 쓰임은 하나다 — **정합 노력의
우선순위 눈금.** §3 의 계층 정합에 들일 다음 한 시간은 주변차 위치의 물리적 충실도가
아니라 ego 동역학과 경로 표현에 써야 한다는 뜻이다.

**ego 안에서도 칸마다 다르다.** 차원별 |z| 를 보면 요각속도 1.03, 좌·우 가장자리 0.83,
**직전 조향 0.76**, 차선 대비 heading 0.76, 속도 0.61 순이고, 맨 끝이 **현재 조향 0.01**
이다. 이 마지막 두 칸의 대비가 구조적 사실을 드러낸다 — MetaDrive 의 ego 4·5 번 칸은
각각 **현재 조향각**(`vehicle.steering`)과 **직전 조향 명령**(`last_current_action[1][0]`)
으로 **서로 다른 양**인데, 경량 커널은 조향에 액추에이터 상태가 없어(명령이 곧 실현)
두 칸에 **같은 값**을 넣는다. 즉 이 구분은 경량 시뮬이 표현할 수 없는 것이고, 타깃에서
현재 조향은 소스 평균에 거의 붙어 있는 반면(0.01) 직전 명령은 0.76σ 떨어져 있다.

이것을 **결함이 아니라 명시되지 않은 단순화**로 적는다. 조향을 즉시 반영하는 모형에서는
두 칸이 같은 것이 내적으로 일관되며, 본 연구는 **이 단순화가 전이에 얼마나 기여하는지
측정하지 않았다**. 다만 §3 의 정합 라운드 기록에 이 항목이 없었고, 의존도가 가장 높은
블록(ego) 안에서 이동이 큰 칸에 해당하므로 **다음 정합의 후보 1순위**로 남긴다.
(학습 커널은 동결 상태이므로 이 서술을 위해 코드를 고치지 않았다 — 시드 비교 가능성이
«ecfe8c6 과 바이트 동일» 에 걸려 있다, §7 배치 효과.)
...
바뀐다. **(b) 도 닫힌다 — 횡방향을 더해도 섭동은 탐색 잡음을 넘지 못한다 (2026-09-23)**.
측정한 횡방향 오차를 종방향과 **상관 ρ=0.50 의 결합 모형**으로 함께 주입하고 같은 감도
측정을 다시 했다(`bench/obs_noise.py --lateral` 경로, `bench/noise_sensitivity.py --lateral`):

| 주입 차원 | \|Δ조향\| (배율 1) | ÷ 탐색 σ | 제로샷 Δ성공률 |
|---|---|---|---|
| 종방향만 | 0.0125 | 0.27 | −0.4%p (p=0.75) |
| **종방향+횡방향** | **0.0150** | **0.33** | −1.5%p (p=0.31) |

차원을 하나 더해도 비율은 0.27 → 0.33 으로 **0.06 오를 뿐**이고, 제로샷 성능은 여전히
잡음 안이다. 배율 4배에서야 1.19 로 등가를 넘지만 그것은 (c) 와 같은 이야기가 된다.
남은 두 차원(요각·속도)을 더하면 어떻게 되는지는 **외삽할 필요가 없다 — 상한이 있다.**

**상한: 값을 통째로 무작위화해도 −2.2%p (2026-09-23, `bench/slot_ablation.py`)**. 주변차
슬롯의 종·횡 값을 검출 반경 안 균등난수로 바꾸면(점유는 유지, 위치 정보는 완전 파괴)
성공률은 81.4% → 79.2% 로 **2.2%p 밖에 떨어지지 않는다**(부호순열 p=0.125). 어떤 인지
오차도 «값을 완전히 모르는 것» 보다 나쁠 수는 없으므로, **값에 잡음을 넣는 어떤 설계도
이 과제에서 2.2%p 이상을 만들 수 없다.** 차원을 몇 개 더하든 마찬가지다.

| 조건 | 성공률 | Δ | 충돌률 |
|---|---|---|---|
| 원본 | 81.4% | — | 16.0% |
| 슬롯 **제거**(전부 0) | 47.1% | **−34.3%p** | 20.2% |
| 슬롯 **무작위**(점유 유지) | 79.2% | −2.2%p | 18.0% |
| navi 블록 무작위 | 22.7% | −58.7%p | 16.7% |
| ego 블록 무작위 | 3.7% | −77.7%p | 5.7% |

(ego·navi 는 0 으로 만들 수 없다 — 학습 중 그런 관측이 없어 정보 제거와 분포 밖 충격이
섞인다. 차원별 **관측 범위 안**에서 다시 뽑아 주변분포는 지키고 정보만 없앴다.)

**그리고 이 표가 이유를 바꾼다.** 슬롯을 통째로 지우면 −34.3%p 이므로 정책은 주변차
정보를 **분명히 쓴다** — 「과제가 주변차를 안 쓴다」가 아니다. 쓰는 것은 **점유**(이 슬롯에
차가 있는가)이지 **그 안의 위치 값**이 아니다(−34.3 대 −2.2, **16배**). 그래서 거리·횡방향
추정 오차는 검출만 성공하면 거의 무해하고, 슬롯이 비면 크게 다친다.

의존의 순서는 **ego(−77.7) > navi(−58.7) > 주변차 점유(−34.3) ≫ 주변차 위치값(−2.2)**
이다. 세 가지 단서와 함께 읽어야 한다. (i) 블록 크기가 다르다(ego 9·navi 10·주변차 32
차원) — 「파괴했을 때의 피해」 순서이지 정규화된 중요도가 아니다. (ii) 범위 안 무작위화는
주변분포를 지키되 **차원 간 상관을 깬다** — 잡음 주입보다 강한 개입이라 상한으로만 읽는다.
(iii) 범위는 무작위 행동으로 조사했으므로 학습된 정책이 실제로 지나는 영역보다 넓을 수
있다. 그럼에도 **주변차 위치값이 최하위라는 순서**는 이 단서들로 뒤집히지 않는다 —
세 가지 모두 개입을 **과하게** 만드는 방향인데도 −2.2%p 에 그쳤기 때문이다.

실무적 함의는 §3 의 계층 정합에 대한 것이다: **정합 노력의 우선순위는 이 순서를 따라야
한다.** 주변차 위치의 물리적 충실도를 높이는 것보다 ego 동역학과 경로 표현을 맞추는 쪽이
이 과제에서는 훨씬 크게 작용한다.

**따라서 «위치 값에 인지 오차를 주입한다» 는 설계는 이 과제에서 성립하지 않는다** —
효과의 상한이 2.2%p 이고 시드 간 산포(σ≈16%p)에 묻힌다. 값싼 경로를 모두 재어 닫았다:
V 상향은 지지집합 붕괴로, 횡방향 추가는 크기 부족으로, 배율 확대는 질문의 변질로, 그리고
차원을 더 재는 길은 **무작위화 상한**으로.

**대신 이 표가 올바른 실험을 가리킨다.** 정책이 읽는 것이 점유라면, 성능을 움직이는
인지 오차는 **회귀 오차가 아니라 검출 오차**(미검출·오검출)다 — 슬롯을 지웠을 때의
−34.3%p 가 그 크기의 눈금이다. §8 (9) 를 「관측 노이즈 주입」에서 **「미검출·오검출률
주입」**으로 바꿔 적는 것이 이 자료가 지지하는 방향이며, 그때 필요한 값은 §6.4 의 회귀
잔차가 아니라 **검출기의 recall/precision** 이다. 본 연구가 남기는 것은 「해 보라」가
아니라 **무엇을 주입해야 하는지를 가려낸 측정**이다.

(b) 의 선행 조건이었던 **횡방향·요각 측정은 2026-09-23 에 같은 원자료로 수행했다(§6.4)**:
