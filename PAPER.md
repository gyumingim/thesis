...
- **좌표 규약 오류는 한 번이 아니었다 (2026-08-29 집계)**: 본 연구가 이 결함 유형을 핵심
  기여로 내세우는 근거는 사례 하나가 아니라 **같은 프로젝트에서 독립적으로 세 번** 나왔다는
  사실이다. (1) 경량 시뮬 커널의 횡방향 기저가 MetaDrive 차량 API 와 거울상 — 전이 평가에서만
  발현해 6주간 in-domain 지표를 통과했다(§4.7). (2) CARLA 폐루프 기록의 실행 회전각을
  `abs()` 로 저장 — 좌·우를 실행 경로로 가를 수 없게 만들었고, 그 표는 스스로 오염됐다고
  선언한 앵커 라벨로 방향을 채우고 있었다(§6.6 표 4). (3) UE 절차 생성기의 정체 열 배치가
  직전 차량의 **카메라 좌표**를 세계 좌표로 사용 — 카메라 요각 지터(±6°) 때문에 중앙값
  1.13 m, 최대 5.0 m(차로 폭의 143%)의 횡 어긋남이 생겼다. 세 사례의 공통점은 (a) 두 좌표계가
  같은 이름의 필드를 쓰고, (b) 축이 정렬된 구간(직진·요각 0)에서 항등사상이라 대부분의 검사를
  통과하며, (c) 값의 분포 통계가 보존된다는 것이다. **따라서 규약 검증은 프로젝트 착수 시
  1회가 아니라 좌표를 다루는 코드가 추가될 때마다 반복해야 한다** — 세 사례 모두 서로 다른
  시점에 서로 다른 파일에서 들어왔다.
