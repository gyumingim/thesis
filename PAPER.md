...
1. **동일 벽시계 교차-시뮬레이터 비교 프로토콜과 그 결과인 예산-성능 교차** — 처리량이 다른
   ...
   34.7배다. 이 프로토콜이 드러낸 것은 승패가 아니라 **교차**다: 동일 장비 대조에서 경량은
   5분 예산에 58.9% 로 네이티브 41.1% 를 17.8%p 앞서고(네이티브가 그 수준에 닿는 데 15분 —
   약 3배 단축), 15분에 교차하며, 1시간에는 48.9% 로 63.3% 에 14.5%p 뒤진다(§6.2).
...
| 계열 | 시드별 Kendall τ | 부호 일치 | Fisher p | 처음 → 끝 |
|---|---|---|---|---|
| 정책 엔트로피 | −0.96 / −1.00 / −0.96 / −1.00 | 4/4 | <0.0001 | +0.05 → −3.30 |
| **소스 수익** | −0.78 / −0.71 / −0.71 / −0.78 | **4/4** | **<0.0001** | 116.8 → 115.9 |
| 에피소드 길이 | −0.53 / −0.75 / −0.78 / −0.93 | 4/4 | 0.0004 | 100.7 → 99.0 |
| 정책 갱신 크기 (approx_kl) | +0.96 / +1.00 / +1.00 / +1.00 | 4/4 | <0.0001 | 0.01 → 0.03 |
| 가치함수 설명력 | 부호 불일치 | — | 0.14 | 0.93 → 0.93 |

엔트로피에서 역산한 행동 표준편차는 **0.221 → 0.047 로 4.8배 축소**된다. 즉 정책은
결정론에 가까워지고, 주행선은 짧아지며(에피소드 길이 −2%), 가치함수는 안정적인데
(설명력 0.93 불변) **소스 수익과 전이 성공률이 함께 내려간다.**

**핵심은 두 하강의 크기 차이다.** 같은 구간에서 소스 수익은 약 1% 줄지만 전이 성공률은
58.9% → 40.0% 로 **19%p** 줄어든다(§6.2). 소스 안에서는 탐색을 꺼도 거의 손해가 없는데
전이에서는 크게 손해 보는 것이다. 이는 §6.2 의 실패 모드 이동(충돌 감소·이탈 증가,
Fisher p=0.0066)과 정합한다 — 결정론적이고 짧아진 주행선은 판정 경계에 더 가깝게 붙고,
경계 밀착의 실패는 충돌이 아니라 이탈이다. 소스에서는 그 경계가 관대해(로드웨이 폭 18 m)
비용이 없지만 타깃에서는 그렇지 않다.

**따라서 철회된 "착취" 해석을 되살리지 않는다.** 정책이 소스의 결함을 찾아 이용하는 것이
아니라(그랬다면 소스 수익이 올라야 한다) **탐색이 꺼지면서 강건성 여유가 사라지는 것**이며,
그 여유는 소스에서 쓸모없고 전이에서만 값을 갖는다. 처방의 방향도 달라진다 — 경계를
보수화하는 것이 아니라 **엔트로피 하한을 유지**하거나(entropy bonus 스케줄) 전반부
체크포인트를 고르는 것이다(§7 (5)의 2단 선택). 다만 엔트로피 하한 처방은 본 연구에서
시험하지 않았으며 §8 향후 연구에 둔다.
...
| 조건 | 전반(≤30분) | 후반(>30분) | Δτ | 전 구간 |
|---|---|---|---|---|
| 부호 오류판 | +0.57 | −0.03 | +0.60 | — |
| GPU 경합 20M | −0.11 | −0.29 | +0.18 | 0.00 |
| **확정 정숙 136M** | **+0.457** (순열 p=0.029) | **+0.053** (p=0.788) | **+0.403** | +0.295 (p=0.103) |

**즉 신뢰 창(trust horizon) 구조는 정정 후에도 실재한다** — 오류판만큼 극적이지는 않지만
(Δτ 0.60 → 0.40), 전반에는 경량 신호로 체크포인트 순위를 예측할 수 있고(순열 p=0.029)
후반에는 무정보로 퇴화한다. 순열은 시드 내 순환 이동이므로 두 시계열의 자기상관을 보존하며,
공통 시간 추세로 설명되지도 않는다(τ(t,M) 이 −0.10/−0.43/+0.02 로 시드마다 부호가 갈린다).

**단, 두 가지 한계를 명시한다.** (i) 경량 학습 수익 L 의 동적 범위가 0.71~1.43 으로 평균
(약 115)의 1% 수준이다 — 사실상 포화된 신호 위의 미세 변동에서 순위를 읽는 것이므로,
같은 구조가 다른 과제에서 재현될지는 별개 문제다. (ii) n=3 시드 × 11 체크포인트이고 전
구간 τ 는 유의하지 않다(p=0.103). 따라서 이 결과는 **DPC 형식의 최초 실증**이되 확정은
아니며, §6.2 의 사후 오라클 조기정지 이득(13.3pp)이 실제로 회수 가능한지 — 즉 전반 구간의
경량 신호만으로 좋은 체크포인트를 고를 수 있는지 — 는 이 τ 를 근거로 **조건부 긍정**으로
답할 수 있다(§7 (5)). 다만 여기의 τ≈0 은 GPU 경합 20M 런에서
...
절차(trust horizon 진단)**다. 이 전이는 부호 오류판(+0.57→−0.03)뿐 아니라 **확정 정숙
조건에서도 관측된다**(+0.457→+0.053, 전반 순열 p=0.029) — 즉 형식의 제안에 그치지 않고
같은 시뮬레이터 쌍에서 정정 후 재현된 실증을 갖는다. 다만 n=3 이고 경량 신호의 동적
범위가 좁으므로, 다른 시뮬레이터 쌍에서의 독립 재현은 여전히 필요하다(§8).
...
- **실무 체크리스트 (본 연구 결과의 함의)**: (0) **규약 검증을 먼저 하라** — 알려진 상대
  ...
  나빴다(§6.2). 다만 62.2% 는 **타깃 라벨을 읽은 상한**이다. 이 상한을 경량
  신호만으로 회수할 수 있는지에는 **조건부로 답할 수 있다** — 같은 정숙 조건의 DPC 재산출에서
  전반(≤30분) 층화 τ 가 +0.457(순열 p=0.029)로 유의하고 후반은 +0.053 로 무정보다(§6.3).
  즉 **학습 전반부에 한해서는 경량 점수로 체크포인트 순위를 고를 수 있고**, 후반부 선택에는
  타깃 평가가 필요하다. 실무 권고는 따라서 (a) 전반부는 경량 신호로 후보를 좁히고,
  (b) 후반부 후보들 사이의 최종 선택만 타깃 소량 평가로 하는 **2단 선택**이다 — 타깃 평가
  비용을 전 구간이 아니라 후반부로 국한할 수 있다. 단 이 처방의 근거인 τ 는 n=3·좁은
  동적 범위 위의 값이므로 잠정이다(§6.3 한계).
