...
48.9% 에 머무른다(§6.2). 다만 이 잔여 격차는 유의성이 성립하지 않고 장비 교차
비교이므로(§2.5·§7), **"충분조건이 아니다"는 잠정 판정**이며 그 기제도 미규명이다.

**검증 (b) — 퇴화 차원 마스킹 (완료)**: 재학습 없이 평가 시 퇴화 차원(학습 σ<10⁻³)만
0으로 마스킹하면 →MetaDrive 전이가 크게 회복된다. **부호 정정판 재측정(2026-08-28, V=2 2시드, GPU 경합 조건 20M)**: 전이 10%/0%(평균 5%)가 마스킹 후 33%/77%(**평균 55%**)로 회복되어, V=3 정상 학습의 확정 전이(48.9%±20.4%, §6.2)와
구간이 완전히 겹친다 — 다만 2시드·σ 31%p 로 분산이 커 우열은 논할 수 없고, 결손의 **주된
원인이 지지집합 위반이었다**는 점만 확증한다. (부호 오류판의 기존 수치 5%→13.5% 는 거울상 관측이 구제 효과를 가린 값이었다.) in-domain 은 86%/89% 로 정상이므로, 차이는 전적으로 관측 지지집합 결손에서 온다. 즉 마스킹은 지지집합
위반의 **주된 피해를 제거**한다. 다만 정정 후에는 마스킹(55%±31%, 2시드)과 V=3 재학습
(48.9%±20.4%, 3시드)의 구간이 겹쳐, "마스킹은 부분 구제이고 재학습이 상위"라는 이전 판의
서열은 더 이상 지지되지 않는다 — 우열 판정에는 시드 보강이 필요하다(§8). 기제 가설로는
...
최종 R6 환경, 144배가 V=16 완성 환경), 53배(노트북)와 **62.6배(데스크톱 정숙)**는 학습을
포함한 실현 비율로 **같은 층위의 서로 다른 장비 값**이다. 두 장비의 절대 SPS 가 크게 다른데
(경량 72K vs 37.8K, MetaDrive 1,360 vs 604) — 데스크톱이 경량 시뮬에서도 MetaDrive 에서도
...
| **경량 V=3 (정정판·정숙 조건, 1h 최종)** | **48.9% ± 20.4%** (27/67/53, 3시드, 136M steps) | 확정치. n=3 에서 IQM 은 퇴화하므로(중앙 50% = 단일 시드) §2.5 의 IQM+구간 표준을 이 행에서는 적용하지 못하고 평균±σ 로 보고한다 |
| 경량 V=3 (정정판·GPU 경합 조건) | 67.8% ± 19.0% (83/73/47, 20M steps) | 표본 수·GPU 경합이 함께 다르므로 확정 행과 단독 대조 불가 |
| 경량 V=3 (부호 오류판) | IQM 0% [0,18] (5시드) | **아티팩트** — §4.7 사례로만 인용 |
| 경량 V=3 최적 조기정지 (오류판, 5시드) | 30% ± 18% | **아티팩트**(오류판 수치) — 정정판의 사후 오라클 값은 시간축 문단 참조 |
...
(에피소드를 시드에 걸쳐 합산한 보조 검정: 53/90 대 36/90, Fisher 정확검정 p=0.0084 —
에피소드 독립 가정에 기대므로 보조로만 본다). 반면 **전 체크포인트에 걸친 성공률의 단조
...
이탈 8.9% → 27.8%, 충돌 32.2% → 25.6%). 짧은 20M 조건에서는 이 이동이 나타나지 않는다 —
하강 구간에 도달하기 전에 예산이 끝나기 때문이다.

**추세 계산의 범위를 명시한다**: 위 τ 와 종점 수치는 시간축이 어긋나는 `final.pt` 를
제외한 11개 체크포인트에 대한 것이다(§6.2 각주 1 의 벡터 워커 스톨 때문에 final 의
벽시계 시각이 시드마다 다르다). `final.pt` 를 포함하면 이탈 8.9% → 18.9%, 충돌
32.2% → 32.2% 로 **이동 폭이 크게 줄고 충돌 감소는 사라진다** — 즉 이 이동은 학습
중반의 현상이며 최종 체크포인트에서 부분적으로 되돌아온다. 이 사실을 숨기면 추세가
실제보다 강해 보인다.
...
- **종합**: 세 처방 중 조기정지만 생존한다. 본 실험 체제(단일 교차로·제로샷/워름스타트)
  ...
  실패 모드 구성의 시간 추세(§6.2, 가설 정합 관찰), 그리고 — 판정 자체는 규약 오류 하의 것이라 인용 불가하나 — 그 처방 시험의 설계가 후속 연구가
...
60% 로 갈렸고 격차는 **전부 좌회전 구간**에서 났으나(11/11 대 5/11, 직진은 양쪽 7/8 로
동일), 두 정책은 각 조건의 최고·최저 시드라 시드 품질이 교락되어 있어 결론으로 삼지
않는다(§8).
...
- **실무 체크리스트 (본 연구 결과의 함의)**: (0) **규약 검증을 먼저 하라** — 알려진 상대
  ...
  나빴다(§6.2). 다만 62.2% 는 **타깃 라벨을 읽은 상한**이며, 경량 신호만 쓰는 정지 규칙은
  정정판에서 전 구간 τ≈0 이라(§6.3) 이 상한을 회수할 수 없다. 따라서 권고는 조기정지가
  아니라 **주기 체크포인트 + 타깃 소량 평가에 의한 선택**, 즉 모델 선택이다.
- **하드웨어 일반화 (2026-08-28 추가)**: 본판은 노트북(RTX 4060)에서, 정정판 재현은
  ...
  **여기서 본 논문 주 결론의 가장 큰 한계를 명시한다**: 확정 전이 48.9% 는 데스크톱(RTX 5080)
  ...
  단, 데스크톱 네이티브는 같은 1시간에 2.17M steps(604 SPS)만 모아 노트북 네이티브
...
68%±9% 에 못 미치며, 첫 체크포인트(5분, 58.9%)가 피크다. 단, n=3·σ=20.4%p 에서 이
격차는 통계적으로 유의하지 않고(Welch t=−1.54, df≈2.4, p≈0.24), 두 수치가 서로 다른
장비에서 얻어졌다는 한계도 남는다(§2.5, §7) — 따라서 이는 **점추정 방향의 잠정 판정**이다.
그러나 이 결론의 **크기는
...
termination semantics, geometry), the calibrated environment steps 138x faster than
MetaDrive without a learning loop (144x for the fully featured V=16 build), and the
headline run realized 62.6x end-to-end including training. Zero-shot transfer from the
lightweight simulator reaches 48.9%+/-20.4%
success against 68%+/-9% for native training -- a point-estimate shortfall rather than the
parity we first reported: the throughput advantage does not convert losslessly into
...
front. With n=3 seeds and sigma=20.4 the gap is not statistically significant (Welch
t=-1.54, df~2.4, p=0.24). And the two numbers come from different machines -- the 48.9%
from a desktop (RTX 5080), the 68% baseline from a laptop (RTX 4060); a single-seed native
run on the desktop under the same protocol scored 53.3%, which narrows the gap to 4.4pp.
...
of fine-tuning by +47.7pp, while the trained policy (from a 20M-step checkpoint, see 6.6) drives closed-loop in a third
simulator (CARLA).
