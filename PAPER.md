...
이 12.7%p 는 평가 시나리오 블록 넷 중 **경량에 가장 유리한 값**이며, 네 블록 평균은
−18.0%p(p=0.094)다 — 시나리오 축의 변동은 시드 축보다 좁아 방향은 유지된다(§7 표 6). 즉 처리량 35.9배는 초반
...
**다만 이 −12.7%p 는 평가 시나리오 블록 넷 중 경량에 가장 유리한 값이다** — 블록
510000·520000·530000 에서는 각각 −18.7·−22.0·−18.7%p 이고 네 블록 평균은 **−18.0%p**
(p=0.094)다(§7 표 6). 헤드라인으로 쓰는 수치가 표본 선택에서 유리한 쪽이라는 점을
여기서 밝혀 둔다. 시나리오 축의 변동 자체는 시드 축보다 좁으므로(σ 비 0.36~0.80)
결론의 방향은 흔들리지 않는다. 시드를 3에서
...
- **평가 시나리오 축 — 측정했다 (2026-09-02 한계 제기 → 2026-09-11 해소)**: 헤드라인 수치는
  ...
  분석 `tools/scenario_axis.py`). 블록 500000·510000·520000·530000 각 30에피소드,
  정책 10종(경량 5 + 네이티브 5) 전수. 원 블록의 재현은 시드별로 정확히 일치했다
  (49.3%·62.0%).
  ...
  **표 6. 평가 시나리오 블록별 팔 평균과 격차** (final.pt, 30에피소드, 결정론).
  ...
  | **격차** | **−12.7%p** | −18.7%p | −22.0%p | −18.7%p | **−18.0%p** |

  세 가지가 나온다. (i) **논문이 쓴 블록의 격차가 네 블록 중 가장 작다** — −12.7%p 는
  경량에 가장 유리한 표본이었고 평균은 −18.0%p 다. 헤드라인 수치는 이 점을 명시해야 한다.
  (ii) 블록 평균으로 평가 잡음을 줄이면 경량 48.3%±8.6 대 네이티브 66.3%±18.1,
  격차 **−18.0%p**(Welch t=2.01, df=5.7, **p=0.094**, 95% CI [−40.2, +4.2])로 단일 블록의
  p=0.227 보다 강해지나 여전히 0.05 를 넘지 않는다 — 검정력 한계(§2.5)가 지배적이다.
  (iii) **시나리오 축은 시드 축보다 좁다**: 팔별 σ 가 경량 시드 12.6 대 시나리오 10.0,
  네이티브 18.7 대 6.7(비 0.80·0.36)이다. 즉 다른 30장면을 썼어도 결론이 흔들리지 않는다는
  것이 이제 수치로 뒷받침되며, 시드 기준 오차막대를 쓴 것도 사후적으로 정당화된다.
  네 블록 모두 격차가 음수라는 점(부호검정 p=0.125, n=4)도 방향의 안정성을 지지한다.

  남는 한계: 블록이 넷뿐이고, 오라클 판정(위 (5))의 독립 가정은 여전히 같은 블록 안
...
(7) **평가 시나리오 축의 복원 — 완료(§7)**. 블록 4개로 분해한 결과 시나리오 축 σ 가 시드 축보다 좁고(비 0.36~0.80) 네 블록 모두 격차가 음수였다. 다만 블록이 넷뿐이므로 블록 수를 늘린 재확인과, 블록 간 난이도 차이의 원인(어떤 시나리오가 어려운가) 규명은 남는다,
