...
1. `bash bench/run_all.sh` 실행 — 약 12~14h. 순차 필수(경합은 벽시계를 오염시킨다).
   MD 3시드 → V=3 3시드 → V=2 3시드 → 3h 확장 → 교차평가 → 마스킹 재평가 → 그림
2. §6.1 처리량 표를 이 장비 실측으로 교체
3. §6.2 `[TBD]` 2곳 소거 (PAPER.md:166 검증(c), PAPER.md:193 V=3 주 결과)

**주의**: 배치가 도는 동안 언리얼 렌더 등 무거운 작업을 겹치면 벽시계 예산이 오염된다.
언리얼 작업을 먼저 끝내고 배치는 그 뒤에 돌린다.
