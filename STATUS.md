...
- **정합 라운드 진행 (2026-08-20)**: R1 부호 → R2 기하·관측 → R3 동역학 → R4 규칙(실선·스폰차선)
  → R5 판정기하(차체 모서리). 상세 PAPER.md 3.11~3.12
- in-domain 84~98%, 전이는 R4에서 리턴 −2→+16 (실주행 시작), 성공률은 R5 재학습 결과 대기
- 다음 의심 지점(R5 통과 시): MetaDrive 교차로 내부 차선 커버리지 (ep5가 내부에서 사망)
- **플로우 A**: UE 데이터 공장 1단계 완료(이미지+GT 검증), CARLA 0.10.0 설치·클라이언트 검증 완료(5080),
  사용자 Fab 에셋 설치 대기
- 단일 에이전트 정책 유지 (HARNESS 반영됨)
