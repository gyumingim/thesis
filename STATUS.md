...
- **프로젝트 구조 확정 (2026-08-20, 사용자)**: 두 플로우 = A(인지: 이미지→차량 3D 위치, 5080/Unreal)
  + B(RL: 경량시뮬, 4060 노트북). 접점 = 객체 리스트 인터페이스. 테슬라 vision-only 구조의 축소판
- **계획 승인 (2026-08-20)**: B1 파일럿→교차평가 → B3 본판(1h×3시드) → B4 노이즈 주입
  / A: 전방 단안 카메라, 차량 3D 위치, 5만 프레임 → 모노 3D 검출 학습 → 오차 통계
- **단, A1(CARLA 설치)은 사용자 지시로 보류** — 플로우 A 데이터 생성 도구 미정 상태.
  A2 이후 단계는 도구 결정 전까지 착수 불가
