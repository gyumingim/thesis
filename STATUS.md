...
검증 세트 3세대 (v3.1, scene_140~149): 판정 위임 후 3심 앙상블(포렌식/사진가/본세션)
2라운드 운영 — 2세대 poor → 처방(노출바이어스·블룸 반감으로 백화 소멸, 태양 물리색온도로
시안 타파, 붕괴 메시 vehCar_vehicle02 퇴출, 정체 열·차선 buffer·원경 2열 봉쇄) → 지표
역대 최저 KID 0.0667·CMMD 3.004 (시각 판정과 첫 동방향). 렌더 4연속 10/10.
판정 자료 교훈: 쌍대 이미지를 축소하면 그레인이 소거돼 노이즈 축 판정이 무효 — 원본 크롭 병용 필수.
남은 티: 파사드 반복, 행인·간판 부재, 차선 가시성.
