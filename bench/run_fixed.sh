...
OUT=bench_results/fixed; mkdir -p $OUT
...
for s in 1 2 3; do
  ...
  $PY bench/ppo.py --sim custom --num-envs 1024 --num-steps 64 --n-vehicles 3 \
    ...
    --seed $s --exp-name fix_custom > $OUT/train_s$s.log 2>&1
done
for d in runs/Intersection__fix_custom__*; do
  [ -d "$d" ] || continue
  s=$(basename "$d" | grep -oE '__[0-9]+__' | tr -d '_')
  $PY bench/evaluate.py --run-dir "$d" --target metadrive --episodes 30 --seed 500000 \
    --out $OUT/eval_md__fix_s$s.json >> $OUT/eval.log 2>&1
  $PY bench/evaluate.py --ckpt "$d/ckpt/final.pt" --target custom --episodes 64 --seed 500000 \
    --n-vehicles 3 --out $OUT/eval_cu__fix_s$s.json >> $OUT/eval.log 2>&1
done
touch $OUT/DONE
